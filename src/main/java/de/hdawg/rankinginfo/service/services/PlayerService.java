...
public class PlayerService {
  ...
  List<Trend> calculateAgeGroupTrends(List<RankingItem> rankingItems, List<String> eligibleAgeGroups) {
    ...
    eligibleAgeGroups.forEach(a -> {
      ...
      if (filtered.size() > 1) {
        ...
        for (RankingItem r : filtered) {
          if (counter % 2 == 0) {
            Trend t = new Trend(r.ageGroup(), r.rankingPosition() - filtered.get(counter + 1).rankingPosition(),
